...
class MRI_SwinMR_NPI(ModelBase):
    ...
    # ----------------------------------------
    def parameters(self):
        """Get parameters from the internal network"""
        return self.netG.parameters()

    # ----------------------------------------
    # forward pass - make model callable
    # ----------------------------------------
    def __call__(self, x):
    ...
    def compute_loss(self, outputs, targets, inputs=None):
        """Compute the loss given outputs and targets"""
        # Store the reconstruction and ground truth for loss computation
        self.E = outputs
        self.H = targets
        self.L = inputs if inputs is not None else torch.zeros_like(inputs)
